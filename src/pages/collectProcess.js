...
export const collectProcessTemplate = (data, formData) => {
                    ...
                    tubes.forEach((obj, index) => {
                        template += `
                            <tr>
                                ...
                                <td>
                                    <input type="text" id="${obj.name}Id" ${formData[`${obj.name}Id`] ? `value='${formData[`${obj.name}Id`]}'`: ''} class="form-control ${formData[`${obj.name}Collected`] === false ? 'disabled': ''} input-barcode-id" ${formData[`${obj.name}Collected`] === false ? 'disabled': 'required'} placeholder="Scan/Type in Tube ID">
    ...
    barCodeBtns.forEach(btn => {
        addEventBarCodeScanner(btn.id, 9, 14, 1);
    });
}
